...
public class BeliefFunctionEstimator2 {

  private final int numTran;
  private final long gracePeriod;
  private final double recencyDiscount;
  ...
  public BeliefFunctionEstimator2(int numTran, long gracePeriod, double recencyDiscount) {
    this.numTran = numTran;
    this.gracePeriod = gracePeriod;
    this.recencyDiscount = recencyDiscount;
    this.bidNotifications = new ArrayList<>();
    this.askNotifications = new ArrayList<>();
    this.transactionNotifications = new ArrayList<>();
  }

  public static BeliefFunctionEstimator2 create(int numTran, long gracePeriod,
      double recencyDiscount) {
    return new BeliefFunctionEstimator2(numTran, gracePeriod, recencyDiscount);
  }
   ...
   */
  public void addOrderWithdrawObservation(OrderNotification notification) {
    if (notification.getOrderType() == OrderType.BUY) {
      orderWithdrawUpdate(bidNotifications, notification);
    } else {
      orderWithdrawUpdate(askNotifications, notification);
    }
  }
  ...
  public void addTransactionObservation(OrderNotification notification) {
    ...
    addOrderWithdrawObservation(notification);
    addOrderWithdrawObservation(o);
  ...
  private int discountPower(OrderNotification order, TimeStamp timeStamp) {
    if (order.getPeriod() != 0) {
      return (int) ((int) (timeStamp.get() - (order.getTimeStamp().get() - order.getPeriod()))
          / 1000);
    } else {
      return (int) ((int) (timeStamp.get() - order.getTimeStamp().get()) / 1000);
    }
  }
   ...
   */
  private double bidsGreater(Price p, TimeStamp timeStamp) {
    int index = bidNotifications.size() - 1;
    double volume = 0;
    while (index != -1 && bidNotifications.get(index).getPrice().compareTo(p) >= 0) {
      OrderNotification temp = bidNotifications.get(index);
      volume =
          volume + temp.getQuantity() * Math.pow(recencyDiscount, discountPower(temp, timeStamp));
      index--;
    }
    return volume;
  }
   ...
   */
  private double asksLess(Price p, TimeStamp timeStamp) {
    int index = 0;
    double volume = 0;
    while (index != askNotifications.size()
        && askNotifications.get(index).getPrice().compareTo(p) <= 0) {
      OrderNotification temp = askNotifications.get(index);
      volume =
          volume + temp.getQuantity() * Math.pow(recencyDiscount, discountPower(temp, timeStamp));
      index++;
    }
   ...
   */
  private double acceptedBidLess(Price p, TimeStamp timeStamp) {
    int index = 0;
    double volume = 0;
    while (index != transactionNotifications.size()
        && transactionNotifications.get(index).getPrice().compareTo(p) <= 0) {
      OrderNotification temp = transactionNotifications.get(index);
      volume =
          volume + temp.getQuantity() * Math.pow(recencyDiscount, discountPower(temp, timeStamp));
      index++;
    }
   ...
   *         acceptedBidGreater
   */
  private double acceptedAskGreater(Price p, TimeStamp timeStamp) {
    ...
    while (index != -1 && transactionNotifications.get(index).getPrice().compareTo(p) >= 0) {
      ...
      volume = volume + transactionNotifications.get(index).getQuantity()
          * Math.pow(recencyDiscount, discountPower(temp, timeStamp));
      index--;
    }
    return volume;
  }
    ...
    while (index != askNotifications.size()
      ...
      if (period > 0 && !transactionNotifications.contains(temp)) {
        ...
        volume += (alivePeriod > gracePeriod ? 1 : (double) alivePeriod / gracePeriod)
            * temp.getQuantity() * Math.pow(recencyDiscount, discountPower(temp, timeStamp));
      }
      index++;
    }
  ...
  private double buyBelief(Price P, TimeStamp timeStamp) {
    double belief = 0;
    if (acceptedBidLess(P, timeStamp) + asksLess(P, timeStamp) != 0) {
      belief =
          (acceptedBidLess(P, timeStamp) + asksLess(P, timeStamp)) / (acceptedBidLess(P, timeStamp)
              + asksLess(P, timeStamp) + rejectedBidGreater(P, timeStamp));
    }
    return belief;
  }
  ...
  private double sellBelief(Price P, TimeStamp timeStamp) {
    double belief = 0;
    if (acceptedAskGreater(P, timeStamp) + bidsGreater(P, timeStamp) != 0) {
      belief = (acceptedAskGreater(P, timeStamp) + bidsGreater(P, timeStamp))
          / (acceptedAskGreater(P, timeStamp) + bidsGreater(P, timeStamp)
              + rejectedAskLess(P, timeStamp));
    }
    return belief;
  }
  ...
  public Pair<Price, Double> estimate(OrderType type, double pVal, Price bBid, Price bAsk,
    ...
    if (type == OrderType.BUY) {
      ...
      if (bBidBelief == 0) {
        ...
        while (i != 0 && buyBelief(buyLow, timeStamp) != 0) {
          i--;
    ...
    // seller
      ...
      if (bAskBelief == 0) {
        ...
        while (i != askNotifications.size() - 1 && sellBelief(sellHigh, timeStamp) != 0) {
          i++;
