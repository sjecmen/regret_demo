...
def sample_spoofing_simulation(strat, mix, emp_min, emp_max):
...
def sample_spoofing_simulation_unnorm(strat, mix): # returns list of player maps, which have strat and payoff
    # realize mix
    profile = np.random.multinomial(spoofing_num_players - 1, mix)
    profile[strat] += 1

    # write spec
    with open('sample_spec.json') as infile: # sample spec must already be created, containing configuration
        spec = json.load(infile)
        config = spec["configuration"]
    bg = {}
    for i, strat in enumerate(spoofing_strategy_names):
        bg[strat] = int(profile[i])
#    assign = {"background":bg, "spoof":{"nsyl":1}}
#    spec = {"configuration":config, "assignment":assign}
